...
// Create the motion.mesh and motion.meshStandardMaterial components
extend({ Mesh: THREE.Mesh, MeshStandardMaterial: THREE.MeshStandardMaterial });
const MotionMesh = motion(THREE.Mesh);
const MotionMeshStandardMaterial = motion(THREE.MeshStandardMaterial);
...
const Neuron = ({ node, onClick, isHighlighted }: { 
  ...
  const materialVariants = {
    initial: { emissiveIntensity: 0.2 },
    animate: { emissiveIntensity: 0.5 },
    hover: { emissiveIntensity: 0.7 },
    highlighted: { emissive: '#ffffff', emissiveIntensity: 0.8 }
  };
  ...
  return (
    <MotionMesh
      ...
      variants={variants}
      initial="initial"
      animate={isHighlighted ? "highlighted" : "animate"}
      whileHover="hover"
    >
      <sphereGeometry args={[node.size || 0.5, 32, 32]} />
      <MotionMeshStandardMaterial
        color={color}
        emissive={color}
...
const Synapse = ({ connection, nodes, highlighted }: { 
  ...
  return (
    <group>
      ...
      <MotionMesh 
        position={curvePoints[Math.floor(progress * curvePoints.length)]}
        initial={{ scale: 0 }}
        animate={{ scale: [0, 1, 0], transition: { duration: 1, repeat: Infinity, ease: "easeInOut" } }}
      >
